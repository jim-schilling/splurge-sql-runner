...
class TestSecurityConfig:
    ...
    def test_is_sql_safe_with_dangerous_sql(self) -> None:
        ...
        assert config.is_sql_safe("EXEC sp_configure 'show advanced options', 1;") is False
    ...
    def test_is_url_safe_with_dangerous_urls(self) -> None:
        ...
        assert config.is_url_safe("data:text/html,<script>alert('xss')</script>") is False
    ...
    def test_custom_validation_config_inheritance(self) -> None:
        ...
        custom_validation = ValidationConfig(
            dangerous_sql_patterns=("CUSTOM_DANGEROUS",),
        ...
        assert config.is_sql_safe("DROP DATABASE test;") is True  # Original pattern not used
