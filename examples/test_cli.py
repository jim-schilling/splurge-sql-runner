...
def test_maintenance():
    ...
    try:
        ...
        result4 = run_cli_command([
            "-c", f"sqlite:///{db_path}",
            "-f", "examples/cleanup_and_maintenance.sql",
            "-v"
        ])
        ...
        if result4.returncode == 0:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='posts_archive'")
...
def main():
    ...
    print("  python -m splurge_sql_runner -c sqlite:///test.db -f examples/basic_setup.sql -v")
